...
resource "azurerm_network_interface" "self_managed_worker_03" {
  name                = "self-managed-worker-03-nic"
  location            = azurerm_resource_group.japan_east.location
  resource_group_name = azurerm_resource_group.japan_east.name
  ...
  ip_configuration {
    ...
    private_ip_address            = "10.3.100.4"
    public_ip_address_id          = azurerm_public_ip.self_managed_worker_03.id
  }
}
