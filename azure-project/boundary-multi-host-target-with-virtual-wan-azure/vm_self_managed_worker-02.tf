# -----------------------------------------------------------------------------
# VM — self-managed-worker-02 (Qatar Central, 192.168.99.0/24)
...
# IP. Uses the same intermediate-remote-key data source as -01 (see
# vm_self_managed_worker-01.tf / vm_intermediate_worker-01.tf).
# -----------------------------------------------------------------------------
...
resource "azurerm_network_interface" "self_managed_worker_02" {
  ...
  ip_configuration {
    ...
    private_ip_address            = "192.168.99.5"
