# VM — intermediate-worker-02 (East US)

resource "azurerm_public_ip" "intermediate_worker_02" {
  name                = "intermediate-worker-02-pip"
...
resource "azurerm_network_interface" "intermediate_worker_02" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.intermediate_worker_02.id
  }
}

# No 9202 inbound: it only dials out to self-managed workers, never listens.
resource "azurerm_network_security_group" "intermediate_worker_02_nsg" {
...
resource "azurerm_network_interface_security_group_association" "intermediate_worker_02" {
  ...
  network_security_group_id = azurerm_network_security_group.intermediate_worker_02_nsg.id
}
