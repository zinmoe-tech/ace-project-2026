#purpose : this is for controlling intermediate-worker-02's network access.
...
resource "aws_security_group" "intermediate_worker" {
  name        = "intermediate-worker-02-sg"
  description = "intermediate-worker-02-sg"
  vpc_id      = aws_vpc.management-network-02-vpc.id

  tags = var.tags
}

#purpose : this is for allowing intermediate-worker-02 to reach HCP
...
#purpose : this is for allowing SSH into intermediate-worker-02 from the
...
resource "aws_security_group" "bastion" {
  name        = "intermediate-worker-02-bastion-sg"
  description = "intermediate-worker-02-bastion-sg"
  vpc_id      = aws_vpc.management-network-02-vpc.id
