#purpose : this is for fetching intermediate-worker-02's self-generated
#auth_request_token so it can be handed to
#`boundary workers create worker-led` without SSHing in by hand. The token
#doesn't exist until boundary-worker.service has actually started on the
#worker — it's written to auth_storage_path on first successful start, not
#baked into the AMI. Reaches the worker by proxying through the bastion,
#the same path a human would use, since the worker has no direct SSH
#ingress from anywhere else.
resource "null_resource" "fetch_worker_auth_token" {
  count = var.create_worker_instance && var.create_bastion_instance ? 1 : 0

  triggers = {
    worker_instance_id = aws_instance.intermediate_worker[0].id
  }
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      WORKER_IP="${aws_instance.intermediate_worker[0].private_ip}"
...
#purpose : this is for reading the fetched token back into Terraform state
#so it can be exposed as an output.
...
#purpose : this is for exposing intermediate-worker-02's auth request token
