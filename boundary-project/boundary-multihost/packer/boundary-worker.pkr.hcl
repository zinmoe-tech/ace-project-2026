...
source "amazon-ebs" "boundary_worker" {
  ...
  #purpose : this is for finding the temporary packer-vpc build VPC by tag,
  #since this account has no default VPC and the VPC/subnet are created by
  #../packer-vpc (a separate Terraform apply, not this Packer run). Apply
  #packer-vpc first, run this build, then terraform destroy packer-vpc —
  #the AMI this produces doesn't depend on packer-vpc still existing after.
  vpc_filter {
    filters = {
      "tag:Name" = "packer-vpc"
    }
  ...
  #purpose : this is for finding the packer-vpc public subnet by tag, so the
  #build instance lands somewhere with a route to the internet gateway.
  subnet_filter {
    filters = {
      "tag:Name" = "packer-vpc-public-sub-01"
    }
    random = false
  }
