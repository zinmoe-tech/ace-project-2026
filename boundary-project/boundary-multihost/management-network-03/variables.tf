variable "region" {
  description = "AWS region hosting management-network-03."
  type        = string
...
variable "vpc_cidr" {
  description = "CIDR block for the management-network-03 VPC."
  type        = string
  default     = "10.3.0.0/16"
}
...
#purpose : this is for sizing the intermediate-worker-03 EC2 instance.
...
#purpose : this is for gating creation of the intermediate-worker-03
...
#to fetch intermediate-worker-03's auth_request_token over SSH.
...
#intermediate-worker-03 is allowed to reach on 9202 — the only egress
