variable "region" {
  description = "AWS region hosting management-network-01."
  type        = string
...
variable "vpc_cidr" {
  description = "CIDR block for the management-network-01 VPC."
  type        = string
  default     = "10.1.0.0/16"
}
...
#purpose : this is for sizing the intermediate-worker-01 EC2 instance.
...
#intermediate-worker-01 with (created out-of-band, .pem lives locally).
variable "worker_key_name" {
  type    = string
  default = "self-managed-worker-01"
}

#purpose : this is for gating creation of the intermediate-worker-01
...
#to fetch intermediate-worker-01's auth_request_token over SSH.
...
#intermediate-worker-01 is allowed to reach on 9202 — the only egress
