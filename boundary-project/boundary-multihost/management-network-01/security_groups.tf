#purpose : this is for controlling intermediate-worker-01's network access.
...
resource "aws_security_group" "intermediate_worker" {
  name        = "intermediate-worker-01-sg"
  description = "intermediate-worker-01-sg"
  vpc_id      = aws_vpc.management-network-01-vpc.id

  tags = var.tags
}

#purpose : this is for allowing intermediate-worker-01 to reach HCP
...
#purpose : this is for allowing SSH into intermediate-worker-01 from the
...
resource "aws_security_group" "bastion" {
  name        = "intermediate-worker-01-bastion-sg"
  description = "intermediate-worker-01-bastion-sg"
  vpc_id      = aws_vpc.management-network-01-vpc.id
