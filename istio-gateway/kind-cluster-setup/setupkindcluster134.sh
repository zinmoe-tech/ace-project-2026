...
kind create cluster --config ./kind-cluster-setup/kindconfig-v134.yaml
sleep 1
kubectl apply -f https://raw.githubusercontent.com/metallb/metallb/v0.15.3/config/manifests/metallb-native.yaml --context kind-134
pods_checking=false
...
while ! $pods_checking; do
    ...
    if kubectl get pods -n metallb-system | grep -i controller | grep -q 'Running' && kubectl get pods -n metallb-system | grep -i speaker | grep -q 'Running' ; then
        ...
        kubectl create -f ./kind-cluster-setup/metallb-v0153-ipaddress-pool-5.yaml --context kind-134
...
sleep 1
kubectl config rename-context kind-134 134
watch kubectl get pods -A --context 134
