...
The current infrastructure uses **exactly two Lambda functions**:

| Lambda Function | Runtime | Purpose | Trigger |
|---|---|---|---|
| `BoundarySessionCounter` | Python 3.12 | Counts active HCP Boundary sessions and publishes `boundary.active_sessions` to Datadog | EventBridge every 1 minute |
| `BoundaryWorkerCleanup` | Python 3.12 | Deletes the matching temporary Boundary worker during ASG scale-in and completes the termination lifecycle action | EventBridge from ASG termination lifecycle event |

![Current Lambda Functions](Current-Lambda-Functions.png)

## Lambda relationship

```text
1. BoundarySessionCounter
...
Vault AWS Auth
        ↓
Read session-monitor + Datadog API key
        ↓
HCP Boundary API
        ↓
...
AWS Lambda — current functions only:
...
No additional Lambda functions are part of the current architecture.
