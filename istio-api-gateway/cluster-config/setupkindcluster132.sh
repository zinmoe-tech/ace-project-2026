...
kind create cluster --config ./kindconfig-v13202.yaml
sleep 1
kubectl apply -f https://raw.githubusercontent.com/metallb/metallb/v0.14.9/config/manifests/metallb-native.yaml --context kind-ace-cluster
pods_checking=false
...
while ! $pods_checking; do
    ...
    if kubectl get pods -n metallb-system | grep -i controller | grep -q 'Running' && kubectl get pods -n metallb-system | grep -i speaker | grep -q 'Running' ; then
        ...
        kubectl create -f ./metallb-v0149-ipaddress-pool-4.yaml --context kind-ace-cluster
...
sleep 1
kubectl config rename-context kind-ace-cluster ace-cluster
watch kubectl get pods -A --context ace-cluster
